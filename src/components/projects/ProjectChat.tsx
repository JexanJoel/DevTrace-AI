...
import type { ProjectChatMessage } from '../../hooks/useProjectCollaboration';
...
const ProjectChat = ({ messages, onSend, currentUserId }: Props) => {
  ...
  useEffect(() => {
    ...
    if (isMyMessage || distanceFromBottom < 100) {
      container.scrollTop = container.scrollHeight;
    }
  }, [messages]);
  ...
  return (
    <div className="flex flex-col bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 overflow-hidden">
      <div className="flex items-center gap-2.5 px-4 py-3 border-b border-gray-100 dark:border-gray-800 flex-shrink-0">
        ...
        <div>
          <p className="text-sm font-bold text-gray-900 dark:text-white">Project Chat</p>
          <p className="text-xs text-gray-400">Team discussion · syncs live via PowerSync</p>
        </div>
      </div>
      ...
      <div ref={containerRef} className="flex-1 overflow-y-auto p-4 space-y-3 max-h-72 min-h-[120px]">
        {messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-20 text-center">
            ...
            <p className="text-xs text-gray-300 dark:text-gray-600">Start the team conversation here</p>
          </div>
        ) : (
          messages.map((msg, i) => {
      ...
      <div className="p-3 border-t border-gray-100 dark:border-gray-800 flex-shrink-0 space-y-2 sm:space-y-0 sm:flex sm:gap-2">
        <input
          value={input}
          onChange={e => setInput(e.target.value)}
          onKeyDown={e => e.key === 'Enter' && !e.shiftKey && handleSend()}
          placeholder="Message the team..."
          className="w-full sm:flex-1 border-2 border-gray-100 dark:border-gray-700 focus:border-indigo-400 text-gray-900 dark:text-white dark:bg-gray-800 rounded-xl px-3.5 py-2.5 text-sm focus:outline-none transition placeholder-gray-300 min-w-0"
        />
        <button
          onClick={handleSend}
          disabled={!input.trim() || sending}
          className="flex items-center justify-center gap-2 w-full sm:w-10 h-10 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl transition disabled:opacity-40 text-sm font-medium flex-shrink-0"
        >
