    ...
    <DashboardLayout title="AI Insights">
      <div className="space-y-6">
        ...
        <div className="rounded-2xl p-5 sm:p-6 flex items-center justify-between flex-wrap gap-4"
          ...
          <div className="min-w-0">
            ...
            <p className="text-indigo-200 text-xs sm:text-sm mt-1">
              {totalAnalyses} analyses · Groq Llama 3.3 70B · local SQLite
            </p>
          </div>
          <button onClick={() => navigate('/sessions')}
            className="flex items-center gap-2 bg-white/20 hover:bg-white/30 border border-white/20 text-white font-semibold px-4 py-2.5 rounded-xl text-sm transition flex-shrink-0">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
          {[
            {
              label: 'Analyses Run', value: totalAnalyses,
              sub: `of ${sessions.length} sessions`,
              icon: <Sparkles size={18} />, color: 'text-indigo-600', bg: 'bg-indigo-50 dark:bg-indigo-950',
            },
            {
              label: 'Avg Confidence', value: `${avgConfidence}%`,
              sub: avgConfidence >= 80 ? '🟢 High' : avgConfidence >= 60 ? '🟡 Medium' : '🔴 Low',
              icon: <Zap size={18} />,
            ...
            {
              label: 'AI Resolution', value: `${aiResolutionRate}%`,
              sub: `${aiResolvedCount} of ${totalAnalyses}`,
              icon: <CheckCircle size={18} />, color: 'text-emerald-600', bg: 'bg-emerald-50 dark:bg-emerald-950',
            },
            {
              label: 'Top Category',
            ...
            <div key={i} className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-4 sm:p-5">
              ...
              <p className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white truncate">{c.value}</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          {/* Category breakdown */}
          <div className="lg:col-span-2 bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-5 sm:p-6">
            <h3 className="font-bold text-gray-900 dark:text-white mb-1 flex items-center gap-2">
              <BarChart2 size={16} className="text-violet-500" /> Error Category Breakdown
            </h3>
            <p className="text-xs text-gray-400 mb-5">How your bugs are classified across all sessions</p>
                  ...
                  <div key={cat}>
                    <div className="flex items-center justify-between mb-1.5 gap-2">
                      <div className="flex items-center gap-2 min-w-0">
                        <span className={`w-2 h-2 rounded-full flex-shrink-0 ${meta.bar}`} />
                        <span className={`text-sm font-medium truncate ${meta.color}`}>{meta.label}</span>
                      </div>
                      <div className="flex items-center gap-2 text-xs text-gray-400 flex-shrink-0">
                        <span className="hidden sm:inline">{totalPct}%</span>
          ...
          <div className="space-y-4">
            <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-5">
              <h4 className="font-bold text-gray-900 dark:text-white text-sm mb-4 flex items-center gap-2">
                <Activity size={14} className="text-blue-500" /> Confidence
              </h4>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-5 sm:p-6">
            ...
            <p className="text-xs text-gray-400 mb-4">Files most frequently flagged by AI</p>
            {topFiles.length === 0 ? (
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-5 sm:p-6">
            ...
            <p className="text-xs text-gray-400 mb-4">Last analyzed sessions</p>
              ...
              {recentAnalyses.map(s => {
                ...
                return (
                  <div key={s.id} onClick={() => navigate(`/sessions/${s.id}`)}
                    ...
                    <span className={`text-xs font-medium px-2 py-0.5 rounded-full flex-shrink-0 hidden sm:inline-flex ${meta.bg} ${meta.color}`}>
                      {meta.label}
                    </span>
                    <span className="text-xs font-bold text-gray-500 flex-shrink-0 w-8 text-right">{conf}%</span>
                  </div>
                );
              })}
            </div>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-5 sm:p-6">
          <h3 className="font-bold text-gray-900 dark:text-white mb-1 flex items-center gap-2">
            <AlertTriangle size={16} className="text-red-500" /> Severity of Analyzed Sessions
          </h3>
          <p className="text-xs text-gray-400 mb-5">Severity breakdown across all AI-analyzed sessions</p>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
            {[
              ...
              <div key={s.key} className={`rounded-2xl border-2 ${s.border} ${s.bg} p-4 text-center`}>
                <p className={`text-2xl sm:text-3xl font-bold ${s.color}`}>{severityCounts[s.key] ?? 0}</p>
